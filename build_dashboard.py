#!/usr/bin/env python3
"""项目管理汇报看板生成脚本 - 7月4日版本"""
import os, json, glob, re, subprocess, tempfile
...
DWS_BIN = os.path.expanduser('~/.real/.bin/dws/bin/dws')

# 钉钉在线文件夹配置（设为 None 则仅使用本地 data 目录）
DINGTALK_FOLDER = "https://alidocs.dingtalk.com/i/nodes/KGZLxjv9VGd7RzOxC6r4vvX1J6EDybno?utm_scene=person_space"

# ===== 从钉钉在线文件夹同步数据 =====
def _call_dws(*args, timeout=30):
    """调用 dws CLI，返回 JSON"""
    result = subprocess.run([DWS_BIN] + list(args), capture_output=True, text=True, timeout=timeout)
    if not result.stdout.strip():
        return {}
    return json.loads(result.stdout)

def sync_from_dingtalk():
    """从钉钉在线文件夹下载 .axls 数据并转换为本地 xlsx 文件"""
    if not DINGTALK_FOLDER or not os.path.exists(DWS_BIN):
        return
    
    # 从 URL 提取 nodeId
    m = re.search(r'/nodes/([A-Za-z0-9]+)', DINGTALK_FOLDER)
    if not m:
        print("⚠️ 无法从钉钉文件夹URL提取nodeId")
        return
    root_node = m.group(1)
    
    print("🔄 从钉钉同步数据...")
    
    # 收集所有项目数据（去重：同名项目取数据量最大的）
    project_data = {}  # {project_name: (headers, rows, row_count)}
    
    # 递归扫描文件夹中的 .axls 文件
    def scan_folder(folder_id, depth=0):
        if depth > 3:
            return
        sub = _call_dws('drive', 'list', '--parent-id', folder_id, '-f', 'json', '--yes')
        for item in sub.get('result', {}).get('items', []):
            if item['type'] == 'FOLDER':
                scan_folder(item['fileId'], depth + 1)
            elif item['name'].endswith('.axls'):
                sheet_data = _call_dws('sheet', 'range', 'read', '--node', item['fileId'], '-f', 'json', '--yes', timeout=60)
                vals = sheet_data.get('result', sheet_data).get('displayValues', sheet_data.get('displayValues', []))
                if len(vals) < 2:
                    continue
                headers = [str(h).strip() if h else '' for h in vals[0]]
                rows = vals[1:]
                pn = _identify_project(rows, headers)
                # 同名项目保留数据量更大的那份
                if pn not in project_data or len(rows) > project_data[pn][2]:
                    project_data[pn] = (headers, rows, len(rows))
    
    scan_folder(root_node)
    print(f"   扫描到 {len(project_data)} 个项目")
    
    synced = 0
    for project_name, (headers, rows, _) in project_data.items():
        os.makedirs(DATA_DIR, exist_ok=True)
        filepath = os.path.join(DATA_DIR, f"{project_name}.xlsx")
        _write_xlsx(filepath, headers, rows)
        print(f"   ✅ {project_name}: {len(rows)} 条任务")
        synced += 1
    
    if synced > 0:
        print(f"✅ 钉钉同步完成，共 {synced} 个项目")
    else:
        print("⚠️ 未找到可同步的数据")

def _identify_project(rows, headers):
    """从数据行中识别项目名"""
    # 查找任务ID列
    id_col = None
    for i, h in enumerate(headers):
        if '任务ID' in h or 'ID' == h:
            id_col = i
            break
    ...
    for row in rows:
        if id_col is not None and id_col < len(row):
            tid = str(row[id_col]).strip() if row[id_col] else ''
        else:
            continue
        # 匹配前缀如 E1W, B30, WR01 等
        m = re.match(r'([A-Za-z]+\s*\d*[A-Za-z]*)', tid)
        if m:
...
# 自动扫描 data 目录下的 xlsx 文件
PROJECT_FILES = {}
for f in sorted(glob.glob(os.path.join(DATA_DIR, '*.xlsx'))):
    fname = os.path.basename(f)
    if fname.startswith('.~'):  # 排除临时文件
        continue
    project_name = os.path.splitext(fname)[0]
    PROJECT_FILES[project_name] = f

PROJECT_NAMES = list(PROJECT_FILES.keys())
print(f"📁 发现 {len(PROJECT_NAMES)} 个项目: {PROJECT_NAMES}")

# ===== 数据读取 =====
def read_project_bugs(filepath):
    """从 xlsx 文件读取任务数据"""
...
for pn in PROJECT_NAMES:
    all_projects_bugs[pn] = read_project_bugs(PROJECT_FILES[pn])
...
milestones = {}
DINGTALK_MILESTONE = "https://alidocs.dingtalk.com/i/nodes/gwva2dxOW4yXp7bMfkbAdRnxVbkz3BRL?utm_scene=person_space"
ms_node = None
ms_match = re.search(r'/nodes/([A-Za-z0-9]+)', DINGTALK_MILESTONE)
if ms_match:
    ms_node = ms_match.group(1)

if ms_node and os.path.exists(DWS_BIN):
    ms_sheet = _call_dws('sheet', 'range', 'read', '--node', ms_node, '-f', 'json', '--yes', timeout=30)
    ms_vals = ms_sheet.get('result', ms_sheet).get('displayValues', ms_sheet.get('displayValues', []))
    if len(ms_vals) > 1:
...
elif os.path.exists(os.path.join(os.path.dirname(__file__), "milestone", "milestone.xlsx")):
    # fallback: 本地文件
    ms_file = os.path.join(os.path.dirname(__file__), "milestone", "milestone.xlsx")
    ms_wb = openpyxl.load_workbook(ms_file, data_only=True)
    for sn in ms_wb.sheetnames:
        ws = ms_wb[sn]
        ...
        for r in range(2, ws.max_row + 1):
            ...
            ms_entry = {
                "date": date_val,
                "name": str(row_data.get("里程碑", "") or ""),
                "item": str(row_data.get("事项", "") or ""),
                "status": str(row_data.get("状态", "") or ""),
                "risk": str(row_data.get("风险", "") or ""),
            }
            if project_name not in milestones:
                milestones[project_name] = []
            milestones[project_name].append(ms_entry)
