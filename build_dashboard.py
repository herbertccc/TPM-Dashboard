...
def _dingtalk_read_large_sheet(node, sheet_id, total_rows, col_start='A', col_end='L', chunk_size=5000, expected_headers=None):
    """分块读取大型表格，返回 (headers, data_rows)
    chunk_size=5000: 减少 API 调用次数，加速读取
...
headers, data_rows = _dingtalk_read_large_sheet(
    ...
    col_start='A', col_end='L', chunk_size=2500,
...
def _read_single_col(col_letter, total_rows, chunk_size=10000):
    """读取单列数据，返回去掉表头后的值列表"""
    col_rows = []
    for start in range(1, total_rows + 2, chunk_size):
        end = min(start + chunk_size - 1, total_rows + 1)
...
# 并行读取 O/R/T/U 公式列，大幅减少总耗时
from concurrent.futures import ThreadPoolExecutor
_formula_cols = ['O', 'R', 'T', 'U']
_col_names = {'O': 'DB-任务状态', 'R': 'DB-DI值', 'T': 'DB-角色', 'U': 'DB-部门'}
with ThreadPoolExecutor(max_workers=4) as pool:
    _col_results = dict(zip(_formula_cols, pool.map(
        lambda c: _read_single_col(c, TOTAL_DATA_ROWS), _formula_cols
    )))
task_status_values = _col_results['O']
di_values = _col_results['R']
role_values = _col_results['T']
dept_values = _col_results['U']
for c in _formula_cols:
    print(f"  📊 {_col_names[c]}列: {len(_col_results[c])} 行")
...
def calc_trend(all_bugs):
    ...
    for i, d in enumerate(dates):
        d_date = datetime.strptime(f"{now.year}-{d}", "%Y-%m-%d")
        total = 0.0
...
.tl-timeline-wrap { display:flex; flex-direction:column; gap:0; margin-top:8px; }
.tl-header-row { overflow-x:hidden; overflow-y:hidden; flex-shrink:0; background:#fafbfc; }
...
.tl-timeline-body { position:relative; padding-bottom:22px; }
...
.tl-names-col .tl-name-spacer { height:2px; }
...
function renderHR() {
  ...
  var roleOrder = {'开发': 0, '软件测试': 1, 'Other': 2};
  ...
  deptOrder.forEach(function(dept) {
    ...
    sections += '<div class="hr-table-wrap">' +
      '<div class="hr-table-header">' +
        '<div class="hr-table-header-dot"></div>' +
        '<div class="hr-table-header-title">' + '全部人员' + '</div>' +
...
function renderTimelinePage() {
  return '<div class="page-header"><div class="page-title">项目大盘</div><div class="page-desc">各项目里程碑计划与进度跟踪</div></div>' +
    '<div class="timeline-section">' +
      ...
      '<div class="tl-timeline-wrap" id="tlWrap">' +
...
function renderVersionPlan() {
  ...
  plans.forEach(function(p) {
    ...
    rows += '<tr>' +
      '<td style="white-space:nowrap;font-weight:600;color:var(--text)">' + (p.date || '-') + '</td>' +
      '<td style="white-space:nowrap;font-weight:700;color:var(--blue)">' + p.name + '</td>' +
      '<td style="font-size:13px;color:var(--text-secondary);line-height:1.6;max-width:500px">' + (overview || '<span style="color:var(--text-placeholder)">-</span>') + '</td>' +
      '<td>' + (p.link ? '<a href="' + p.link + '" target="_blank" style="display:inline-block;padding:3px 10px;background:var(--blue);color:#fff;border-radius:4px;font-size:11px;text-decoration:none;font-weight:600">链接</a>' : '<span style="color:var(--text-placeholder)">-</span>') + '</td>' +
      '</tr>';
  });
  return '<div class="page-header"><div class="page-title">版本计划</div><div class="page-desc">各项目版本开发计划</div></div>' +
    '<div class="hr-table-wrap"><table class="hr-table">' +
    '<thead><tr><th>日期</th><th>版本</th><th>内容</th><th>链接</th></tr></thead>' +
...
function _renderTimelineInner(viewport, projects) {
  ...
  // 日期列头部（独立行，置于 viewport 外部）
  headerHtml += '<div class="tl-day-cols" style="width:' + innerWidth + 'px">';
  for (var i = 0; i < totalDays; i++) {
    var d = new Date(renderStart.getTime() + i * 86400000);
    var isToday = d.getTime() === today.getTime();
    var isDataRange = d >= tlDataStart && d <= tlDataEnd;
    headerHtml += '<div class="tl-day-col' + (isToday ? ' today-col' : '') + '" style="width:' + tlDayWidth + 'px">';
    headerHtml += '<div class="tl-day-label">' + (d.getMonth()+1) + '/' + d.getDate() + '</div>';
    headerHtml += '</div>';
  }
  headerHtml += '</div>';
  ...
  // 组装：独立日期头行 + viewport（固定名称列 + 可滚动时间线）
  var wrap = document.getElementById('tlWrap');
  var headerRow = document.getElementById('tlHeaderRow');
  if (!headerRow) {
    headerRow = document.createElement('div');
    headerRow.id = 'tlHeaderRow';
    headerRow.className = 'tl-header-row';
    wrap.insertBefore(headerRow, viewport);
  }
  ...
  scrollArea.addEventListener('scroll', function() {
    ...
    headerRow.scrollLeft = scrollArea.scrollLeft;
    syncing = false;
  });
  ...
  var initScroll = Math.max(earliestPx, Math.min(defaultStartPx, latestPx));
  scrollArea.scrollLeft = initScroll;
  headerRow.scrollLeft = initScroll;
...
function resetTimeline() {
  ...
  setTimeout(function() {
    ...
    if (scrollArea && tlRenderStart) {
      ...
      if (headerRow) headerRow.scrollLeft = targetLeft;
    }
  }, 100);
}
