...
def build_project(name, titles, target_di):
    """
    构造Bug列表使OPEN DI精确等于target_di
    策略：先放一批已关闭/已解决的零分Bug填充数量，再精确凑DI值
    """
    bugs = []
    remaining_di = target_di
    idx = 0

    # 阶段1: 用 P0/P1 待处理Bug凑整数部分
    while remaining_di >= 10 and idx < len(titles):
        bugs.append(make_bug(name[:3].upper(), idx, titles[idx % len(titles)],
                              "P0", "待处理", random.choice(PERSONS), random.randint(0, 14)))
        remaining_di -= 10
        idx += 1

    while remaining_di >= 3 and idx < len(titles):
        bugs.append(make_bug(name[:3].upper(), idx, titles[idx % len(titles)],
                              "P1", "待处理", random.choice(PERSONS), random.randint(0, 14)))
        remaining_di -= 3
        idx += 1

    while remaining_di >= 1 and idx < len(titles):
        bugs.append(make_bug(name[:3].upper(), idx, titles[idx % len(titles)],
                              "P2", "待处理", random.choice(PERSONS), random.randint(0, 14)))
        remaining_di -= 1
        idx += 1
    ...
    for _ in range(p3_count):
        if idx < len(titles):
            bugs.append(make_bug(name[:3].upper(), idx, titles[idx % len(titles)],
                                  "P3", "待处理", random.choice(PERSONS), random.randint(0, 14)))
            idx += 1

    # 阶段3: 补充已关闭/已解决的零分Bug，使总数达到25条左右
    closed_statuses = ["已关闭", "已解决"]
    while len(bugs) < 25:
        level = random.choices(["P0", "P1", "P2", "P3"], weights=[0.2, 0.3, 0.3, 0.2])[0]
        status = random.choice(closed_statuses)
        bugs.append(make_bug(name[:3].upper(), idx, titles[idx % len(titles)],
                              level, status, random.choice(PERSONS), random.randint(0, 14)))
        idx += 1
...
def main():
    ...
    for name, titles, target_di in [
        ...
        projects.append({"id": name, "name": name, "bugs": bugs})

    fake_json = json.dumps(projects, ensure_ascii=False)
    html = html.replace(
        ']};\n// 里程碑 sheet',
        '}, ' + fake_json[1:] + '];\n// 里程碑 sheet'
    )
